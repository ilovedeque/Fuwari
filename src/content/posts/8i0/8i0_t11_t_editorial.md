...
Tóm tắt đề bài: Cho mảng $a$ gồm $n$ phần tử. Tính $\max(a) \times \min(a)$ sau khi xoá đi phần tử $a_{n}$.
...
Tóm tắt đề bài: Cho mảng $a$ gồm $n$ phần tử. In ra các phần tử sau khi xoá đi các phần tử trùng lặp và đã giữ bản sao cuối cùng.
...
}; // đoạn code này dùng để tránh trường hợp O(n^2) của unordered_set

const int N = 1e5 + 1;
int n, a[N];
unordered_set <int, custom_hash> s; // cấu trúc dữ liệu giữ lại 1 bản sao của 1 phần tử khác nhau
vector<int> r;
...
Tóm tắt đề bài: Cho mảng $a$ gồm $n$ phần tử. Tính khoảng cách từ phần tử đầu tiên đến vị trí đầu tiên sao cho số lượng phần tử khác nhau tính đến thời điểm đó bằng $m$.
...
Tóm tắt đề bài: Cho mảng $a$ gồm $n$ phần tử. Đếm số lượng phần tử mà xuất hiện từ $2$ lần trở lên.
...
}; // đoạn code này dùng để tránh trường hợp O(n^2) của unordered_map

int n, s, cnt = 0;
unordered_map <int, int, custom_hash> mp; // cấu trúc dữ liệu dùng làm mảng đếm
