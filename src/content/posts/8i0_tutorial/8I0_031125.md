...
## 1. Ý tưởng
Với đứa trẻ thứ `i`, ta có thể chia cho nó số kẹo từ 0 đến `a[i]`.

Nếu tổng số kẹo hiện tại đã được chia là `x`, thì đến đứa trẻ thứ `i`, ta có thể chia được `x` đến `x + a[i]` kẹo.

Phát biểu ngược lại, ta có nhận xét sau:

- Để có thể có được `x` viên kẹo sau khi chia kẹo cho đứa trẻ thứ `i`, ta phải chia cho đứa trẻ từ 0 đến `a[i]` viên kẹo từ tổng số `x` đến `x - a[i]` tổng số kẹo đã cho tương ứng.
...
for(int i = 1; i <= n; i ++) {
    // chuẩn bị pfs
    for(int j = 1; j <= k; j ++) pre[j] = (pre[j] + pre[j - 1]) % mod;
